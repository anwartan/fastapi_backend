...
@router.get("/")
def get(session: SessionDBKafeLogin,search: str = None,limit: int = 10, offset: int = 0,current_user: dict = Depends(get_current_user)):
    query = select(Member).offset(offset).limit(limit)
    if search:
        query = query.where(Member.Nama.like(f"%{search}%"))
    results = session.exec(query).all()
    query_total = select(func.count(Member.ID))
    if search:
        query_total = query_total.where(Member.Nama.like(f"%{search}%"))
    total = session.exec(query_total).first()
    return {"data": results,
            "paging": {
                "limit": limit,
